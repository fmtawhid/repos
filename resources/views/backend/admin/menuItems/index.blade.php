...
@section('title')
    {{ localize('Menu List') }} {{ getSetting('tab_separator') }} {{ getSetting('system_title') }}
@endsection
@section("pagetitle", localize('Manage Menu Item'))

@section('breadcrumb')
    @php
        $breadcrumbItems = [
            ["href"  => null, "title" => localize("Menu Item List")]
        ];
    @endphp
...
@section('pageTitleButtons')
    <div class="col-auto">
        <x-form.button type="button" id="addMenuItemBtnForOffCanvas" data-bs-toggle="offcanvas"
                       data-bs-target="#addMenuItemSideBar">
            <i data-feather="plus" class="icon-14"></i>{{ localize('Add Menu Item') }}
        </x-form.button>
    </div>
@endsection
...
@section('content')
                            ...
                            <table class="table tt-footable border rounded" data-use-parent-width="true">
                                <thead>
                                    <tr class="bg-secondary-subtle">
                                        ...
                                        <th>{{localize('Item Category')}}</th>
                                        <th>{{localize('Addons')}}</th>
    ...
    <!-- Add User start-->
    @include('backend.admin.menuItems.form-menu-item')
    <!-- Add User -->
...
@section('js')
    ...
    @include('backend.admin.menuItems.js')
