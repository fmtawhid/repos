...
@section('title')
    {{ localize('Privacy Policy') }} {{ getSetting('tab_separator') }} {{ getSetting('system_title') }}
@endsection
@section("pagetitle", localize('Privacy Policy'))

@section('breadcrumb')
    @php
    $breadcrumbItems = [['href' => null, 'title' => localize('Privacy Policy')]]; @endphp
                ...
                <div class="col-md-4 col-xl-3">
                    <div class="card">
                        <nav class="navbar navbar-expand-md p-0 tt-settings-nav">
                            <a class="navbar-brand d-lg-none d-md-none" href="#"> {{ localize('Privacy Policy') }} </a>
                                    ...
                                    <div class="card-body">
                                        <div class="tab-content">
                                            <form action="{{ route('admin.settings.store') }}" method="POST" class="copy-write-text-form settingsForm" enctype="multipart/form-data" id="copy-write-text-form">
                                                ...
                                                <div class="row g-3">
                                                    <div class="col-12">
                                                        <h6 class="mb-0 text-center">{{localize('Privacy Policy')}}</h6>
                                                    </div>
                                                    <div class="mb-3">
                                                        <x-form.label for="privacyPolicy" label="{{ localize('Privacy Policy') }}" />
                                                        <x-form.textarea name="settings[privacyPolicy]" id="editor" class="editor"
                                                                    type="text"
                                                                    placeholder="{{ localize('write something here ....') }}"
                                                                    value="{!! html_entity_decode(systemSettingsLocalization('privacyPolicy', $lang_key)) !!}"
                                                                    showDiv=false
                                                        />
